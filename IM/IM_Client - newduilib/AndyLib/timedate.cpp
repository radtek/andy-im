...
int andyDateTime::GetYear()
{
	GetTime();
	return (int)m_time.wYear;	
}

int andyDateTime::GetMonth()
{
	GetTime();
	return (int)m_time.wMonth;
}

int andyDateTime::GetDay()
{
	GetTime();
	return (int)m_time.wDay;
}

int andyDateTime::GetDayOfWeek()
{
	GetTime();
	return (int)m_time.wDayOfWeek;
}

int andyDateTime::GetHour()
{
	GetTime();
	return (int)m_time.wHour;
}

int andyDateTime::GetMinute()
{
	GetTime();
	return (int)m_time.wMinute;
}

int andyDateTime::GetSecond()
{
	GetTime();
	return (int)m_time.wSecond;
}

//毫秒：既然要调用到这个函数，目的当然是得到
//实时的系统时间，因此不管是否设置为与系统时间绑定，
//均取系统时间
int andyDateTime::ms()
{
	GetTime();
	return (int)m_time.wMilliseconds;
}
...
LPCTSTR andyDateTime::year2str(int length/*=0*/)
{
	if(length == 2)
...
LPCTSTR andyDateTime::month2str(int length/*=0*/)
{
	return formatStr(m_str+5,month(),length);
}

LPCTSTR andyDateTime::day2str(int length/*=0*/)
{
	return formatStr(m_str+8,day(),length);
}

LPCTSTR andyDateTime::hour2str(int length/*=0*/)
{
	return formatStr(m_str+11,hour(),length);
}

LPCTSTR andyDateTime::minute2str(int length/*=0*/)
{
	return formatStr(m_str+14,minute(),length);
}

LPCTSTR andyDateTime::second2str(int length/*=0*/)
{
	return formatStr(m_str+17,second(),length);
}

LPCTSTR andyDateTime::formatStr(LPTSTR s,int nval,int length)
{
	if(length != 2)
		wsprintf(s,_T("%d\0"),nval);
	else
		wsprintf(s,_T("%02d\0"),nval);
	return s;
}

BOOL andyDateTime::SetDate(WORD month,WORD day,WORD year,BOOL bSetSystem)
{
	GetTime();
   ...
   if(year > 1979 && year < 2100)		
		m_lptime->wYear = year;
   if(month > 0 && month < 13)
   	m_lptime->wMonth = month;
   if(day > 0 && day < 32)
   	m_lptime->wDay = day;

	if(m_bReCount && bSetSystem)
		return SetLocalTime(&m_time);
	return TRUE;
}

BOOL andyDateTime::SetTime(WORD hour,WORD minute,WORD second,BOOL bSetSystem)
{
	if(bSetSystem)
		GetTime();
   ...
   if(hour < 24)
   	m_lptime->wHour = hour;
   if(minute < 60)
   	m_lptime->wMinute = minute;
   if(second < 60)
   	m_lptime->wSecond = second;
...
andyDateTime& andyDateTime::operator=(const andyDateTime& rdt)
{
	if(this != &rdt)
	{
		m_bReCount = rdt.m_bReCount;
		memcpy(&m_time,&rdt.m_time,sizeof(SYSTEMTIME));
		memcpy(&m_tmptime,&rdt.m_tmptime,sizeof(SYSTEMTIME));
		m_lptime = rdt.m_lptime;
	}
	return *this;
}

andyDateTime& andyDateTime::operator=(const SYSTEMTIME systime)
{
	m_bReCount = FALSE;
	memcpy(&m_time,&systime,sizeof(SYSTEMTIME));
	memcpy(&m_tmptime,&systime,sizeof(SYSTEMTIME));
	m_lptime = &m_tmptime;
	return *this;
}

andyDateTime& andyDateTime::operator =(const FILETIME filetime)
{
	m_bReCount = FALSE;
	m_lptime = &m_tmptime;
	FileTimeToSystemTime(&filetime,m_lptime);
	return *this;
}

//转换到文件时间
BOOL andyDateTime::GetFileTime(LPFILETIME lpft)
{
	if(lpft)
		return SystemTimeToFileTime(m_lptime,lpft);
	else
		return FALSE;
}

SYSTEMTIME* andyDateTime::IncMonth(int nStep,SYSTEMTIME* lptime/*=NULL*/)
{
	m_bReCount = FALSE;
	...
	if(lptime == NULL)
	{
		...
		memcpy(lptime,&m_time,sizeof(SYSTEMTIME));
	}

	int mon = lptime->wMonth + nStep;
	...
	lptime->wMonth = mstep;
/*
	if(ystep <= 0)	//nstep < 0，年份往前推
	{
		time.wMonth = 12 + mstep;
		time.wYear += ystep - 1;
	}
	else
	{		
...
//可识别替换字符（大小写敏感）：
//yyyy或yy:年份，4位或2位
//m或mm:月份（1~12）或（01~12）
//d或dd:日（1~31）或（01~31）
//h或hh:小时（0~23）或（00~23）
//n或mi:分钟（0~59）或（00~59）
//s或ss:秒（0~59）或（00~59）
//ms:毫秒（3位）
//w或ww:星期（英文，Mon,Tus...或全称)
//W:星期（中文）
//ampm:时间转换为12小时制，ampm替换为AM或PM
//AMPM:时间转换为12小时制，AMPM替换为上午或下午
//
//szFormat为空，采用默认时间格式：yyyy-mm-dd hh:mi:ss
//
**********************************************************/
/*LPCTSTR andyDateTime::FormatString(LPCTSTR szFormat)
{
	if(szFormat)
	{
		m_str = szFormat;
...
BOOL CCalendar::Create(RECT &rect, CWnd *pParentWnd, UINT nID)
{
	...
	m_oPopMenu.AppendMenu(MF_STANDY, ID_TITLEBKCOLOR, _T("标题背景色..."));
	m_oPopMenu.AppendMenu(MF_STANDY, ID_TITLECOLOR,   _T("标题前景色...")); 
	m_oPopMenu.AppendMenu(MF_STANDY, ID_FORECOLOR ,   _T("日期前景色..."));
 	m_oPopMenu.AppendMenu(MF_STANDY, ID_SELCOLOR,     _T("选中区颜色..."));
							   ...
							   LONG startx, LONG starty)
{
	 ...
	 if(iDay++ == m_iDay)
	 {
		dc.FillRect(&CDuiRect(startx-1, starty, 
